...
if (process.env.AUTH_TEST_TOKEN) {
  ...
  if (response.status !== 200) {
    throw new Error(`valid token list: expected 200, received ${response.status}`);
  }

  const data = await response.json();
  if (!Array.isArray(data)) {
    throw new Error('valid token list: expected JSON array');
  }

  console.log(`valid token list: ${response.status}`);
} else {
  console.log('valid token list: skipped because AUTH_TEST_TOKEN is not set');
}
